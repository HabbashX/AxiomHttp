 ...
 *
 * <p>Instances are created by {@link RequestRegistry#assemble}
...
public class RequestMethod {
    ...
    public RequestMethod(HttpClient client, String uri, String body,
                         Map<String, List<String>> headers, BuildStrategy strategy) {
        this.client   = client;
        this.uri      = uri;
     ...
     */
    public ResponseContext execute() throws Exception {

        HttpResponse<String> response =
                client.send(
                        strategy.apply(freshBuilder(), body),
    ...
    private HttpRequest.Builder freshBuilder() {
        ...
        if (headers != null && !headers.isEmpty()) {
            builder.headers(HeaderParser.toHeaderArray(headers));
        }
        return builder;
    }

    private ResponseContext toResponseContext(HttpResponse<String> response) {
        return new ResponseContext(
                response.body(),
                response.headers().map(),
                response.statusCode()
